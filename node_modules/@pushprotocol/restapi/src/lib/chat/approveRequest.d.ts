import { EnvOptionsType, SignerType } from '../types';
import { IPGPHelper } from './helpers';
export interface ApproveRequestOptionsType extends EnvOptionsType {
    /**
     * Chat request sender address
     */
    senderAddress: string;
    pgpPrivateKey?: string | null;
    /**
     * Request state. As of now, only `Approved` is allowed
     */
    status?: 'Approved' | 'Reproved';
    account?: string | null;
    signer?: SignerType | null;
}
/**
 * Approve Chat Request
 */
export declare const approve: (options: ApproveRequestOptionsType) => Promise<string>;
export declare const approveCore: (options: ApproveRequestOptionsType, pgpHelper: IPGPHelper) => Promise<string>;
